defmodule AreaRoom.F do
  ...
  defp process({ amount, _ }) when amount < 0, do: { :error, "No negatives allowed!.  Try again." }
  defp process({ amount, units }) when units in ["f", "m"], do: { :ok, Measurement.new(amount, units) }
  defp process({ _, _ }), do: { :error, "You must use the proper form.  Try again.  Example input: 23.8f or 48.13m" }
  defp process(:error), do: { :error, "You didn't enter a valid amount.  Try again." }
